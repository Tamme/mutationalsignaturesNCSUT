...
def train_rfn_cpu(X, n_hidden, n_iter, learnrateW, learnratePsi, dropout_rate):
    n, m = X.shape
    k = n_hidden
    W = np.absolute(np.random.normal(scale=0.01, size=(k, m)).astype(np.float32))
    P = np.array([0.1] * m)
    ...
    for cur_iter in range(n_iter):
        ...
        #M-step
        Cii = C - np.dot(-2*U + np.dot(W.T, S), W) #error
        dP = np.diag(Cii) - P
        #print np.amin(H)
        #print '\n'
        #if cur_iter != n_iter - 1:
        #    W = np.maximum(W, 0.0)
        #W = W*0.99
        ...
        if cur_iter % 1000 == 0:
            print "iter %3d (elllapsed time: %5.2fs)" % (cur_iter, time.time() - t0)
	#print H
    return W, H, P
