...
void read_corpus(char* data_filename, corpus* c, int depth)
{
    ...
    outlog("read doc %s", data_filename);
...
///*
        ...
        //outlog("read countc %d", nunique);
        //outlog("read word %d", word);
...
double gem_score(corpus* corp)
{
	...
	//outlog("scooore1 %f", vget(curr_doc->tot_levels, depth-1));
	//outlog("scooore11 %f", last_log_prob);
	//outlog("scooore111 %f", curr_doc->score);
        score += curr_doc->score;
    }
    //outlog("scooore %f", score);
