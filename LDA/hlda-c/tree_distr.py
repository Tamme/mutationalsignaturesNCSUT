...
#    return
    ...
    if giveDistr == True:
        if sum(topic) != 0:
	        return([i/float(sum(topic)) for i in topic])
        else:
            return([0 for i in topic])
    else:
...
def read_state(state_filename,
    ...
    for line in state:
        ...
        tree[id]['full_distr'] = [id] + top_n_words(topic, vocab, sig_size, True)
        print tree[id]['full_distr']
        print 'id',id
#        print 2/float(100)
        tree[id]['children'] = []
...
def write_topic_tree_ascii(state,
    ...
    out_distr = file(out_filename + '.distr', 'w')
    tree = state['tree']
    ...
    def write_topic(topic, level):
        ...
        out.write(' %s\n\n' % ' '.join([x.upper() for x in topic['top_words']]))
	     ...
	     #my hack
        #print 'TOPIC', topic['full_distr']
        distrStr = list(map(str, topic['full_distr']))
        out_distr.write(str(level) + ' ' + ' '.join(distrStr) + '\n')
...
if (__name__ == '__main__'):
    ...
    main(type, iter_filename, vocab_filename, dmap_filename, out_filename, ndocs = 13)
