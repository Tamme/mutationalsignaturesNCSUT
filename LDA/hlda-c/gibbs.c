...
void init_gibbs_state(gibbs_state* state)
{
	...
	//outlog("compute score  : %d", i);
    compute_gibbs_score(state);
	//outlog("compute score  done: %d", i);
}
                                    ...
                                    char* out_dir)
{
    ...
    if (out_dir != NULL)
    {
		...
		//outlog("koooook");
		//outlog("SCORREOOOOORE1122 %06d %14.3f %14.3f %14.3f %14.3f",
          //      best_state->iter, best_state->gem_score, best_state->eta_score,
            //    best_state->gamma_score, best_state->score);		
		//getchar();
...
void set_up_directories(gibbs_state * state, char * out_dir)
{
    ...
    outlog("settings up7 %s", filename);
    int l;
